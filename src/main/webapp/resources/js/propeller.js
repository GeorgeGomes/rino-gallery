...
(function (w) {
    ...
    var Propeller = function (element, options) {
        ...
        defaults.randomAngleStop = this.getRandomArbitrary(this.angleInit, this.angleEnd);
        this.update();
    };
	...
	p.getRandomArbitrary = function (min, max) {
    ...
    p.updateAngle = function () {
           ...
           /* console.log("Angulo Atual:"+this._angle);
            ...
            if(this.speed > 3.4 && this.speed < 3.6 && defaults.qtdeVoltas == 0){
                ...
                if(parseInt(defaults.randomAngleStop) >= (parseInt(this._angle)-3) && parseInt(defaults.randomAngleStop) <= (parseInt(this._angle)+3)){
                    ...
                    this.speed = this.speed - compensar; 
                    defaults.qtdeVoltas = 1;                   
                }
            }
            ...
            /*if(this.speed > 3.4 && this.speed < 3.6 && defaults.qtdeVoltas == 0){
                ...
                if(parseInt(this._angle) > -1 && parseInt(this._angle) < 10){
                    alert(parseInt(this._angle));
                }
                if(parseInt(this._angle) == parseInt(0)){
                  alert("igual")
                    defaults.qtdeVoltas = 1;                   
                }
            }*/

            /*if(this.speed > 0.900000 && this.speed < 0.9999999 && defaults.qtdeVoltas == 0){
                this.speed = 0.9888888; 
		...
		//console.log("this.angleInit"+defaults.angleInit)
		//console.log("this.angleEnd"+defaults.angleEnd)
    ...
    p.applyInertia = function () {
        if (this.inertia > 0) {
            if (Math.abs(this.speed) >= this.minimalSpeed) {
                ...
                VELOCIDADE = this.speed * this.inertia;
				
				console.log("this.speed = " + (this.speed * this.inertia));
    ...
    p.updateAngleToMouse = function (event) {
        ...
        if (this.stepTransitionTime !== defaults.stepTransitionTime) {
            ...
            VELOCIDADE = this.mouseDiff = this.differenceBetweenAngles(mouseDegrees, this.lastMouseAngle);
            ...
            VELOCIDADE = this.differenceBetweenAngles(newAngle, oldAngle);
        }
    }
    ...
    p.initOptions = function (options) {
        ...
        this.angleInit = options.angleInit || defaults.angleInit;        
        this.angleEnd = options.angleEnd || defaults.angleEnd;        
